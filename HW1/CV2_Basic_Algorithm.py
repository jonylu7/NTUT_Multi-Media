...
def drawRectanglesAndText(image, f, rectangle_coordinates, rectangle_color, rectangle_thickness,
                          ...
                          text_color: (255, 255, 255), text_thickness: 1, text_scale: 0.8):
    ...
    showImage("label_" + f, image)


def morpbologyEx(binary_image, f, kernel, morphMode: cv2.MORPH_ERODE):
    ## cv2.MORPH_DILATE cv2.MORPH_OPEN cv2.MORPH_CLOSE
    eroded_image = cv2.morphologyEx(binary_image, morphMode, kernel)
    showImage("morpbology_" + f, eroded_image)
    return eroded_image


def greythreshold(grey_image, f, threshold_value: 0, max_value: 255, method):
    if (method == "SELF"):
    ...
    showImage("binarization_" + f, thresholded_image)
    return thresholded_image


def bFilter(image, f, d, sigmaColor, sigmaSpace):
    filtered_image = cv2.bilateralFilter(image, d, sigmaColor, sigmaSpace)
    showImage("filter_" + f, filtered_image)
    return filtered_image


def blur(image, f, kernel_size, method):
    if (method == "G"):
    ...
    showImage("filter_" + f, blurred_image)
    return blurred_image


def convertImageToGrey(image, f):
    grey_image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    showImage("gray_" + f, grey_image)
    return grey_image
